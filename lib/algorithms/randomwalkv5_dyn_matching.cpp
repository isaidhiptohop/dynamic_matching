...
randomwalkv5_dyn_matching::randomwalkv5_dyn_matching (dyn_graph_access* G, double eps) : dyn_matching(G) {
    ...
    #ifdef DM_COUNTERS
        counters::create("rw_v5");
        counters::get("rw_v5").create("match()");
    #endif
...
bool randomwalkv5_dyn_matching::new_edge(NodeID source, NodeID target, double& elapsed) {
    timer t;
...
bool randomwalkv5_dyn_matching::remove_edge(NodeID source, NodeID target, double& elapsed) {
    timer t;
    ...
    if (isMatched(source, target)) {
        ...
        ASSERT_TRUE(freeVertex(source));
        ASSERT_TRUE(freeVertex(target));
        ...
        settle(source);
        if (freeVertex(source)) {
            solve_conflict(source, 0);
        }
        
        if (freeVertex(target)) { // the random walk on source could match target
                                  // therefore only settle target, if it's still free
            settle(target);
            
            if (freeVertex(target)) {
...
std::vector<std::pair<NodeID, NodeID> > randomwalkv5_dyn_matching::getM () {
...
/*
dyn_graph_access randomwalkv5_dyn_matching::getM () {
    return M;
}
*/
// checks whether a vertex is free
bool randomwalkv5_dyn_matching::freeVertex (NodeID node) {
    return M.getEdgesFromNode(node).size() == 0;
}

// checks whether an edge is matched or not
bool randomwalkv5_dyn_matching::isMatched (NodeID source, NodeID target) {
///*
...
void randomwalkv5_dyn_matching::match (NodeID u, NodeID v) {
    ...
    #ifdef DM_COUNTERS
        counters::get("rw_v5").get("match()").inc();
    #endif
}

void randomwalkv5_dyn_matching::unmatch (NodeID u, NodeID v) {
    ASSERT_TRUE(isMatched(u, v));
    ASSERT_TRUE(M.getNodeDegree(u) == 1);
    ASSERT_TRUE(M.getNodeDegree(v) == 1);
    M.remove_edge(u, v);
...
NodeID randomwalkv5_dyn_matching::getMatchedEdge (NodeID source) {
...
void randomwalkv5_dyn_matching::solve_conflict (NodeID u, int step) {
...
void randomwalkv5_dyn_matching::settle (NodeID u) {
...
void randomwalkv5_dyn_matching::counters_next() {
    #ifdef DM_COUNTERS
        counters::get("rw_v5").get("match()").next();
    #endif
}
