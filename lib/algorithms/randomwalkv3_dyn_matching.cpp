...
randomwalkv3_dyn_matching::randomwalkv3_dyn_matching (dyn_graph_access* G) : dyn_matching(G) {
    ...
    #ifdef DM_COUNTERS
        counters::create("rw_v3");
        counters::get("rw_v3").create("match()");
    #endif
...
bool randomwalkv3_dyn_matching::new_edge(NodeID source, NodeID target, double& elapsed) {
    timer t;
...
bool randomwalkv3_dyn_matching::remove_edge(NodeID source, NodeID target, double& elapsed) {
    timer t;
...
std::vector<std::pair<NodeID, NodeID> > randomwalkv3_dyn_matching::getM () {
...
/*
dyn_graph_access randomwalkv3_dyn_matching::getM () {
    return M;
}
*/
// checks whether a vertex is free
bool randomwalkv3_dyn_matching::freeVertex (NodeID node) {
    return M.getEdgesFromNode(node).size() == 0;
}

// checks whether an edge is matched or not
bool randomwalkv3_dyn_matching::isMatched (NodeID source, NodeID target) {
///*
...
void randomwalkv3_dyn_matching::match (NodeID u, NodeID v) {
    ...
    #ifdef DM_COUNTERS
        counters::get("rw_v3").get("match()").inc();
    #endif
}

void randomwalkv3_dyn_matching::unmatch (NodeID u, NodeID v) {
    ASSERT_TRUE(isMatched(u, v));
    ASSERT_TRUE(M.getNodeDegree(u) == 1);
    ASSERT_TRUE(M.getNodeDegree(v) == 1);
    M.remove_edge(u, v);
...
NodeID randomwalkv3_dyn_matching::getMatchedEdge (NodeID source) {
...
void randomwalkv3_dyn_matching::solve_conflict (NodeID u, int step) {
    // do this recursion maximal sqrt(m) times or break if u is matched
    if (step >= (std::sqrt(G->number_of_edges())) && freeVertex(u)) {
        settle(u);
        return;
    }
...
void randomwalkv3_dyn_matching::settle (NodeID u) {
...
void randomwalkv3_dyn_matching::counters_next() {
    #ifdef DM_COUNTERS
        counters::get("rw_v3").get("match()").next();
    #endif
}
