...
int parse_parameters(int argn, char **argv, Config & config, file_reader & freader, unsigned & multi_run) {
        ...
        struct arg_int *step                                 = arg_int0(NULL, "step", NULL, "step size to read graph snapshots");
        struct arg_int *_multi_run                           = arg_int0(NULL, "multi-runs", NULL, "amount of runs");
        struct arg_end *end                                  = arg_end(100);

        // Define argtable.
        void* argtable[] = {
                help, filename, user_seed, step, _multi_run, end
        };
        ...
        if(filename->count > 0) {
                freader.directory = filename->sval[0];
        }
